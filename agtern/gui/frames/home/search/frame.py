...
class InternshipSearchFrame(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self._major = ttk.OptionMenu(
            self,
            self.majorInput,
            self.MAJOR_OPTIONS[5],
            *self.MAJOR_OPTIONS,
            command=self._onselect_major,
        )
        self._major.config(width=25)
        # 1.5 Semester?
        self.semesterInput = tk.StringVar()
        self._semester = ttk.OptionMenu(
            self,
            self.semesterInput,
            self.SEMESTER_OPTIONS[1],
            *self.SEMESTER_OPTIONS,
            command=self._onselect_semester,
        )
        # 1.6 Year?
        self.yearInput = tk.StringVar()
        self._year = ttk.OptionMenu(
            self,
            self.yearInput,
            self.YEAR_OPTIONS[0],
            *self.YEAR_OPTIONS,
            command=self._onselect_year,
        )
